...
import "../erc-1155/ERC1155NFTon.sol";
import "@openzeppelin/contracts/proxy/BeaconProxy.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

/**
 * @dev This contract is for creating proxy to access ERC1155NFTon token.
 *
 * The beacon should be initialized before call ERC1155NFTonFactoryC2 constructor.
 *
 */
contract ERC1155NFTonFactoryC2 is Ownable{
    address public beacon;
    address transferProxy;
    address lazyTransferProxy;

    event Create1155NFTonProxy(address proxy);
    event Create1155NFTonUserProxy(address proxy);
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, uint salt) external {        
        ...
        ERC1155NFTon token = ERC1155NFTon(beaconProxy);
        token.transferOwnership(_msgSender());
        emit Create1155NFTonProxy(beaconProxy);
    }
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, uint salt) external {
        ...
        ERC1155NFTon token = ERC1155NFTon(address(beaconProxy));
        token.transferOwnership(_msgSender());
        emit Create1155NFTonUserProxy(beaconProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC1155NFTon(0).__ERC1155NFTon_init.selector, _name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC1155NFTon(0).__ERC1155NFTonUser_init.selector, _name, _symbol, baseURI, contractURI, operators, transferProxy, lazyTransferProxy);
    }
